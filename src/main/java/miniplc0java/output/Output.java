...
public class Output {
    ...
    private void transferFunction(Function function){
        ...
        //2.ret_slots
        addInt(4, function.getRetType());
        //3.param_slots
        addInt(4, function.getParamNum());
        //4.loc_slots
        addInt(4, function.getLocalVNum());
        //5.body.count
        addInt(4, function.getInstructions().size());
        //6.body.items
        List<Instruction> instructions = function.getInstructions();
        for(Instruction instruction:instructions){
            String op = instruction.getOpt();
            //指令
            ...
            if(instruction.getX() != null){
                //只有push的操作数是64位
                if(opInt == 1)
                    addLong(8, instruction.getX().longValue());
                else
