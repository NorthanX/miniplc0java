...
public class StringIterator {
    ...
    Scanner scanner;
    // 指向下一个要读取的字符
    Position ptrNext = new Position(0, 0);
    ...
    public StringIterator(Scanner scanner) {
    ...
    public Position previousPos() {
        ...
        if (ptr.col == 0) {
            return new Position(ptr.row - 1, linesBuffer.get(ptr.row - 1).length() - 1);
        }
        return new Position(ptr.row, ptr.col - 1);
    }
